...
class RDDNeck(nn.Module):
	def __init__(self, dilation, in_channels, out_channels, down_flag, relu=False, projection_ratio=4, p=0.1):
		...
		self.conv2 = nn.Conv2d(in_channels = self.reduced_depth, out_channels = self.reduced_depth, kernel_size = 3, stride = self.stride, padding = self.dilation, 
...
class ENet(nn.Module):
	def __init__(self, C=1, out_c = 3):
		...
		self.b10 = RDDNeck(dilation=1, in_channels=13+C, out_channels=64, down_flag=True, p=0.01)
		...
		self.b28 = RDDNeck(dilation=16, in_channels=128, out_channels=256, down_flag=False)
		
		# The third bottleneck
		self.b31 = RDDNeck(dilation=1, in_channels=256, out_channels=256, down_flag=False)
		
		self.b32 = RDDNeck(dilation=2, in_channels=256, out_channels=256, down_flag=False)
		
		self.b33 = ASNeck(in_channels=256, out_channels=256)
		
		self.b34 = RDDNeck(dilation=4, in_channels=256, out_channels=256, down_flag=False)
		
		self.b35 = RDDNeck(dilation=1, in_channels=256, out_channels=256, down_flag=False)
		
		self.b36 = RDDNeck(dilation=8, in_channels=256, out_channels=256, down_flag=False)
		
		self.b37 = ASNeck(in_channels=256, out_channels=256)
		
		self.b38 = RDDNeck(dilation=16, in_channels=256, out_channels=256, down_flag=False)
		
		# The fourth bottleneck
		self.b40 = UBNeck(in_channels=256, out_channels=64, relu=True)
		...
		self.b50 = UBNeck(in_channels=64, out_channels=13+C, relu=True)
		
		self.b51 = RDDNeck(dilation=1, in_channels=13+C, out_channels=13+C, down_flag=False, relu=True)
		
		# Final ConvTranspose Layer
		self.fullconv = nn.ConvTranspose2d(in_channels=13+C, out_channels=out_c, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False)
...
# print(model.getnumberofparams(model))
