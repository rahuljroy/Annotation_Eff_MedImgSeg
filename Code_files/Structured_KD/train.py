...
wandb.init(project='KD_ENet_spine', 
        name='ENet_ce_'+str(0)+'_'+str(0)+'_'+str(args.n_epochs),
        reinit=True,
        config = args
)
...
for epoch in range(args.n_epochs):
    ...
    torch.save(model.student.state_dict(), 'teacher_model/spine_teacher_'+str(epoch)+'.pt')
...
for lamb1 in lambda1:
    ...
    wandb.init(project='KD_ENet_spine', 
            name='ENet_pi_'+str(lamb1)+'_'+str(0)+'_'+str(args.n_epochs),
            reinit=True,
            config = args
    )
...
for lamb1 in lambda1:
    for lamb2 in lambda2:
        ...
        wandb.init(project='KD_ENet_spine', 
                name='ENet_pa_'+str(lamb1)+'_'+str(lamb2)+'_'+str(40),
                reinit=True,
                config = args
        )
