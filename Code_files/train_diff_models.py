...
def train_UNet(input_type, output_type, wandb, train_loader, validation_loader, weights, opt, device):
    epochs = opt.n_epochs
    ...
    model, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model, wandb, epochs, num_classes, weights, train_loader, input_type, output_type, columns, validation_loader, optimizer, scheduler, part = 'f')
...
def train_MOUNet(input_type, output_type_phase1, output_type_phase2, wandb, train_loader, validation_loader, weights, opt, device):
    ...
    model1, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model1, wandb, epochs1, num_classes_1, weights[0:2], train_loader, input_type, output_type_phase1, columns, validation_loader, optimizer, scheduler, part = 'l')
    ...
    model_enc, model_dec1, model_dec2, avg_losses, avg_dscoeffs, dscoeffs_train, \
     ...
     weights, train_loader, validation_loader, input_type, output_type_phase1, output_type_phase2, columns, \
         optimizer0, optimizer1, optimizer2, scheduler0, scheduler1, scheduler2, 0.5)
...
def train_NFTNet(input_type, output_type_phase1_left, output_type_phase1_right, output_type_phase2, wandb, train_loader, validation_loader, weights, opt, device):
    epochs1 = opt.n_epochs_phase1
    epochs2 = opt.n_epochs_phase2
    num_classes_1 = opt.n_classes_phase1
    num_classes_2 = opt.n_classes_phase2
    ...
    model1_left, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model1_left, wandb, epochs1, num_classes_1, weights[0:2], train_loader, input_type, output_type_phase1_left, columns, \
            validation_loader, optimizer_left, scheduler_left)
    ...
    model1_right, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model1_right, wandb, epochs1, num_classes_1, weights[0:2], train_loader, input_type, output_type_phase1_right, columns, \
            validation_loader, optimizer_right, scheduler_right, part = 'r')
    ...
    model2, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model2, wandb, epochs2, num_classes_2, weights, train_loader, input_type, output_type_phase1, columns, validation_loader, optimizer, scheduler, part = 'f')
...
def train_SubNet(input_type, output_type_phase1_left,  output_type_phase2_full, wandb, train_loader, validation_loader, weights, opt, device):
    epochs1 = opt.n_epochs_phase1
    epochs2 = opt.n_epochs_phase2
    num_classes_1 = opt.n_classes_phase1
    num_classes_2 = opt.n_classes_phase2
    ...
    if num_classes_1 == 2:
        columns=["Background", output_type_phase1_left, "Avg Dice"]        
    model1_left, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model1_left, wandb, epochs1, num_classes_1, weights[0:2], train_loader, input_type, output_type_phase1_left, columns, validation_loader, optimizer_left, scheduler_left)
    ...
    if num_classes_1 == 2:
        columns=["Background", output_type_phase1_right, "Avg Dice"]        
    model1_right, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model1_right, wandb, epochs1, num_classes_1, weights[0:2], train_loader, input_type, output_type_phase1_right, columns, validation_loader, optimizer_right, scheduler_right)
    ...
    if num_classes_2 == 2:
        columns=["Background", output_type_phase2, "Avg Dice"]        
    model2, avg_losses, avg_dscoeffs, dscoeffs_train, table_train, val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
        train_model(model2, wandb, epochs2, num_classes_2, weights, train_loader, input_type, output_type_phase1, columns, validation_loader, optimizer, scheduler)
