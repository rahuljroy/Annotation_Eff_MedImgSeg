...
def train_model(model, wandb, epochs, num_classes, weights, train_loader, input_type, output_type, columns, validation_loader, optimizer, scheduler, phase = '1', part = 'l'):
    ...
    for epoch in range(1, epochs+1):
        ...
        wandb.log({"Train Avg loss"+phase+'_'+part: round(avg_loss/len(train_loader), 3), "Train Avg dice"+phase+'_'+part: round(avg_dscoeff/len(train_loader), 3)})
        ...
        val_avg_losses, val_avg_dscoeffs, val_dscoeffs, table_val = \
            validate_model(model, wandb, num_classes, weights, validation_loader, input_type, output_type, table_val, part)
        pbar_outer.update(1)
...
def train_val_mounet(model_enc, model_dec1, model_dec2, wandb, epochs, num_classes_1, num_classes_2,\
     weights, train_loader, validation_loader, input_type, output_type1, output_type2, columns, optimizer0, optimizer1, optimizer2, scheduler0, scheduler1, scheduler2, Lambda=0.5):
